...
// ─── ВАЖНО: API ключи для приватных эндпоинтов ───────────────────────────────
// Dev:        VITE_* переменные из .env.local (только для разработки)
// Production: те же VITE_* из .env.production
//             Запросы с HMAC подписью идут напрямую из браузера к бирже.
//             Если биржа блокирует CORS для приватных эндпоинтов — добавим
//             Nginx proxy или C# контроллер точечно после проверки.
// ─────────────────────────────────────────────────────────────────────────────
