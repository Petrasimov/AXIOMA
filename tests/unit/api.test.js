...
describe('enrichOpportunities — фильтрация спреда', () => {
  ...
  it('отрицательный спред (bid < ask) — запись отсеивается', async () => {
    ...
    // Формула в api.js: (bid_price - ask_price) / bid_price * 100
    // Чтобы спред был отрицательным: bid < ask
    // bid=1900, ask=2000 → (1900-2000)/1900 = -5.26% → отсеивается ✅
    const records = [makeRawRecord({
      bid: [['1900', '100']],   // bid ниже (SELL) — нет арбитража
      ask: [['2000', '100']],   // ask выше (BUY)  — ask > bid → spread < 0
    })]
  ...
  it('спред > 50% — запись отсеивается как нереалистичная', async () => {
    ...
    // Формула: (bid_price - ask_price) / bid_price * 100
    // Чтобы спред был > 50%: ask < bid * 0.5
    // bid=2000, ask=900 → (2000-900)/2000 = 55% > 50% → отсеивается ✅
    const records = [makeRawRecord({
      bid: [['2000', '100']],
      ask: [['900', '100']],    // спред = (2000-900)/2000*100 = 55% > 50%
    })]
...
describe('enrichOpportunities — арбитражная логика (КРИТИЧНО)', () => {
  it('КРИТИЧНО: при strategy=sf, spot НИКОГДА не должен быть bid_ex', async () => {
    ...
    if (result.length > 0) {
      const r = result[0]
      // bid_ex = "binance" (без суффикса), bid_market = "futures"
      // ask_ex = "mexc"    (без суффикса), ask_market = "spot"
      // parseExchange() отрезает суффикс — проверяем через market-поля
      expect(r.bid_market, 'bid_market должен быть futures при strategy=sf').toBe('futures')
      expect(r.ask_market, 'ask_market должен быть spot при strategy=sf').toBe('spot')
      expect(r.bid_ex, 'SPOT не может быть bid_ex при strategy=sf').not.toBe('spot')
    }
  })
