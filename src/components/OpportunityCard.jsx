...
const style = `
  ...
  /* ══════════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════════
     Внутри карточка уже полностью на flex (без фиксированных сеточных
     колонок) — панели Sell/Buy и так стоят одна под другой, поэтому
     ужимается сама по себе без структурных правок. Реальная проблема
     ширины была снаружи, в OpportunityGrid.jsx (minmax 360px) —
     подробности там. Здесь только увеличиваем тач-таргеты мелких
     иконок-кнопок (было ~17px кликабельной зоны — мало для пальца).
  */
  @media (max-width: 1024px) {
    /* Расширяем тач-таргет только по вертикали. Горизонтально
       соседние кнопки (избранное/скрыть) разделяет всего 8px gap —
       расширение вбок дало бы им пересекающиеся зоны тапа, легко
       промахнуться в "скрыть" вместо "избранное". Вертикально места
       достаточно, коллизий нет. */
    .card-icon-btn { padding-top: 8px; padding-bottom: 8px; margin-top: -8px; margin-bottom: -8px; }
  }
`
